...
subheader: "How to turn point estimtates into probability distributions!"
date: "2024-12-31"
category: tutorials
---
...
All image generation consists of is being able to accurately approximate the distribution of the images in our training dataset. If we're then able to sample images for this approximated dataset, we have theoretically "generated" an image, i.e. we have a brand new image that seems like it could've come from our training dataset, but in fact is actually brand new. 

Naturally, you might be a little concerned, given that "approximating the distribution" of our training images seems a little complicated. With the correct framing, we see that stable diffusion finds its roots from variational inference. 
...
![hi](/blogAssets/images/vi_lr/Pasted_image_20241214195928.png)
...
Fitting a line to this dataset is pretty trivial with PyTorch. Here's the `Model` class and the training loop for it below.
...
![hi](/blogAssets/images/vi_lr/Pasted_image_20241215175724.png)
...
So now let’s spice things up. Right now, our model just outputs a point prediction for any given $x$. That doesn’t entirely reflect the data as you can see because the datapoints of larger $x$ values are much more spread out than with smaller $x$ values. So what if instead of point estimates, our model outputted a distribution? If we choose a Gaussian, that means for any given $x$, our model outputs two values: a mean and a standard deviation. This will allow our model to not just capture the general trend the y-values exhibit, but also the *aleatoric uncertainty* or the inherent noise present in the data generation process.

The changes for this are pretty simple. All we need to do is modify our network such that it outputs two values for a prediction rather than one. We assume the first column is the prediction mean and the second column is the `raw_std`. Since we want our standard deviation to always be positive, we pass the this raw value through the `Softplus` function. `Softplus` is essentially a smoothed out version of `ReLu`, seen below.

![hi](/blogAssets/images/vi_lr/Pasted_image_20241215193313.png)
...
Now the other major change is our loss function. We're replace the `MSELoss` with the negative log-likelihood.

The way we measure if our model's outputted distribution for a given $x$ input is accurate is we want to note how "likely" the associated y value in our training data is under this predicted distribution. This value is known as the "likelihood" and is simply just computing the value of the distribution's PDF, parametrized by the model's predicted mean & standard deviation. The higher the likelihood, the better as we assume that to mean the ground truth y value having come from the predicted distribution to be more realistic or believable.

![hi](/blogAssets/images/vi_lr/Pasted_image_20241216122319.png)
...
Additionally, it's typically easier to work with logs because of numerical stability (the log makes numbers that are close to zero slightly more spread out), and since the log function is strictly monotonically increasing, it doesn't affect our optimization. Also, multiplication & division become addition & subtraction in log-space, making calculating derivates a breeze.

Also because our goal is to *maximize* the likelihood, but PyTorch automatically minimizes any given loss function, we multiply the log-likelihood by -1. 

The `log_prob` function for any distribution in PyTorch just computes the value of the log of the distribution's PDF.

# The Code
So what does this look like in code? This is what our model will look like!
```python
class Model(torch.nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
And since our model outputs a `Normal` distribution object, we can use its built in `log_prob` function to evaluate the likelihood of our training data.
...
If I plot the results, we get the following! The green lines are two standard deviations away from the mean.

![hi](/blogAssets/images/vi_lr/Pasted_image_20241216122817.png)

What we're utilizing here can also be referred to the *Maximum Likelihood Estimator* should you pursue other resources!

# Small Aside on MAP (Maximum a Posteriori)

In the previous example, we incorporated no prior beliefs about the distribution of our y-values. MLE can be handy, but may also fall short with limited data or run the risk of overfitting on our data. Additionally, if we have any prior beliefs regarding what we think the distribution of our training y-values might look like, then why not give that to our model? The more information, the better, right?

Earlier, we were optimizing $w^{MLE} = \underset{w}{\mathrm{argmax}} \log P(D|w)$. We were choosing our weights such that the likelihood of our data was the greatest. If we choose to add a prior over our output distribution $P(w)$, we can formulate our loss to be $w^{MAP}= \underset{w}{\mathrm{argmax}} [\log P(D|w) + \log P(w)] = \underset{w}{\mathrm{argmax}} \log P(w|D)$. We got this from Bayes Theorem which states that $P(w|D)=\frac{P(D|w)P(w)}{P(D)}$, or $\text{posterior} = \frac{likelihood \times prior}{evidence}$ Since $P(D)$ isn't a function of $w$ it wasn't included in the argmax. This is also known as the *Maximum a Posteriori* objective.

The issue is that in a lot of cases, this posterior distribution, $P(w|D)$ is intractable. Solving for it requires many samples $w$ to estimate, which can get computationally infeasible. And unfortunately, there's no neat closed-form solution.
...
![FirstContourPlot](/blogAssets/images/vi_lr/Pasted_image_20241217130703.png)

Let's check your answer with some simply PyTorch code!
...
\end{align*}
$$
Additionally, we can also estimate it by taking repeated samples from $q(x)$.
$$
\begin{align*}
...
The KL divergence between our variational posterior and true posterior can be written as $D_{\text{KL}}(q(w|\theta) \| P(w|D))$. Since we cannot compute our true posterior, let's convert this equation into a form that's more easier to manage. 

*Need to clean/double check this derivation, use `Bayes by Backprop.ipynb` derivation.* Need to also add intermediate clarifications.
$$
\begin{align*}
D_{\text{KL}}(q(w|\theta) \| P(w|D)) &= \int q(w|\theta)\log \left(\frac{q(w|\theta)}{P(w|D)}\right)dw\\
\end{align*}
$$
Remember that $P(w|D)=\frac{P(D|w)P(w)}{P(D)}$.
$$
\begin{align*}
...
\end{align*}
$$
All we did was use the property of logs, $\log ab = \log a + \log b$ and $\log \frac{a}{b} = \log a - \log b$, to break up the integral into three parts.
...
\end{align*}
$$
And since the KL divergence is always zero or positive, we can drop $D_{KL}(q(w|\theta)||P(w|D))$ to give us a *lower bound on the log-evidence*.
...
\end{align*}
$$
 Now hopefully the *evidence lower bound* name makes sense! It's the core equation we're trying to optimize in variational inference. You'll see it anywhere where we're trying to fit a distribution to some data by maximizing its likelihood.
...
![hi](/blogAssets/images/vi_lr/Pasted_image_20241217172818.png)

Comparing to the old version, the regularization force that the KL divergence term plays is quite clear. The standard deviation lines are much wider apart, signifying the model's reluctance to risk overfitting to the dataset.
# The Reparameterization Trick

There is one final issue we need to discuss. The current model is retuning a `Distribution` object in PyTorch that we can easily use to calculate the closed form solution to our loss. However, sometimes we may need to use a sampled point from said distribution. This is prevalent in more advanced use cases of VI, such as with variational autoencoders. 
...
![hi](/blogAssets/images/vi_lr/Pasted_image_20241217172231.png)
...
This means any loss function where we use the reparametrized estimate (the first return value of our `forward` function), we can easily calculate the loss of with respect to our model's parameters!
...
Here, we've manually coded the log-likelihood function for the Gaussian distribution, which is:
$$
\begin{align*}
...
I plan on turning this blog post into a YouTube video, and then my next topic that I'll tackle will be autoencoders! In the meantime, take care and please reach out should there be any questions.

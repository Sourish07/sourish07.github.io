...
It's through this process of passing a data point forward, comparing it with the ground truth, calculating the error, and backpropagating the error through the network in reverse, and updating the weights that the weights will eventually start yielding accurate predictions. It's the fundamental backbone to all of the systems today that use a neural network(s) under the hood.
...
The derivatives of the first linear transformation is the same as the one previously computed. The derivative with respect to $$W_1$$ is $$x$$ and the derivative with respect to $$b_1$$ is $$1$$. We can use these pieces to calculate the partials with respect to $$W_1$$ and $$b_1$$.

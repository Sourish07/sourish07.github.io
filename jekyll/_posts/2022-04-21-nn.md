...
We call $$-(y-a_2)\sigma'(z_2)$$ the error term for the output layer. The gradient for any neuron is its layer's error term times the input of that specific neuron. The error term is useful to calculate separately because error terms in earlier layers are dependent on error terms in later layers. The backpropagation algorithm is named as such because we "propagate" the error backwards through the network to calculate the gradients.

Note that the gradient of the bias weight is just the error term itself. This makes sense because the value of the bias term is $$1$$.
...
Again, the error term of the hidden layer is $$-(y-a_2)\sigma'(z_2)W_2R'(z_2)$$. If there were any more hidden layers before this one, we would've repeated this process to calculate the error terms for those layers.

<!-- Because we're in the realm of Linear Algebra, it helps to outline the expected dimensions of the values in our gradients, as a sanity check.

$$
\begin{align*}
-(y-a_2)&\rightarrow(3 * 1)\\
\sigma'(z_2)&\rightarrow(3*1)\\
W_2\rightarrow()
\end{align*}
$$ -->

...
The testing phase is when we give the trained network the test data, data it hasn't seen before, to guage how well it performs on data it hasn't seen before. It simply performs a forward pass on each test dataset and compares the network's prediction to the ground truth for that test example. It counts how many times its prediction was right and gives us an accuracy at the end.

*The weights of the network are not tuned when pass in the test data!*

## Linear algebra

Let’s first review the basics of Linear Algebra again. This branch of mathematics provides us with some very minimal notation. Take a look at the diagram of a neuron again. 

![Neuron](../assets/images/nn/NNAdvancedNeuron.svg)

The output of the neuron is the weighted sum of the inputs, passed into the activation function. Let's call it $$f(x)$$ for now.
...
Activation function introduce non-linearaity to the neural network. This gives the network the ability to solve problems beyond those that a basic linear regression could solve.

![Activation Functions](../assets/images/nn/activation_funcs.svg)
...
I won't derive each of the functions by hand, but I've inserted the derivatives below.
...
One more activation function I want to mention is the softmax function. It doesn't have a graph, because it's input is a vector of the layer's neurons.
...
No one knows which activation function is the best one. It's usually dependant on the specific use case. I would recommend starting with the ReLu for the hidden layers and the softmax activation function for the output layer. Then, taking into account factors such as accuracy of the network and training speed, see if modifying the activation functions yield any improvements.

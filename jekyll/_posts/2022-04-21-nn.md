...
If you look closely at the line in the third model above, you'll see that it reaches a local maximum when x is 4 and starts to decline. It correctly predicts the data points it has already seen, but cannot predict the new point correctly, even though it's a point that fits the trend of the dataset.

Think about a student that memorizes the study sheet for an upcoming test and then is unable to answer questions that are conceptually similar, but slightly different from the questions they've seen previously. Whereas a more ideal approach might be to not just memorize the answers on the study sheet, but rather to understand the concepts required to solve them.
...
The exact process is repeated to get the values for the output layer. The weight matrix for the output layer, $$W_2$$ will be a 3 x 2 matrix. Each row in $$W_2$$ represents the weights for each neuron in the output layer. Each column represents a neuron from the hidden layer. The output layer will have a bias vector of length 3. Let's call the values in the hidden layer $$h$$ and the activation function for the output layer $$g(x)$$ .
...
Activation functions also need to be differentiable. This allows us to calculate the gradients of the cost function and minimize it. The ReLu is unique because it's not differentiable at 0, but we just manually set the derivative there to be 0.
...
If we wanted to, we could write the whole network on one line. It's probably clear why we don't though. It'll get increasingly messy as the number of layers increase.
...
Let's start with our cost function, Mean Squared Error. Because $$a_2$$, (written as $$\hat y$$ above), is the value that's dependent on our weights and biases, we want to take the partial derivative with respect to that.
